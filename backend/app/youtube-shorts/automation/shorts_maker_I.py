...
from helper.image import generate_images_parallel, create_image_clips_parallel, create_image_clip_with_zoom
...
class YTShortsCreator_I:
    ...
    def create_youtube_short(self, title, script_sections, background_query="abstract background",
                            ...
                            custom_background_path=None, custom_background_clips=None):
        """
        Create a YouTube Short with the given script sections and image backgrounds.

        Args:
            title (str): Title of the video
            script_sections (list): List of dict with text, duration, and voice_style
            background_query (str): Search term for background images
            output_filename (str): Output filename, if None will be generated
            add_captions (bool): Whether to add captions to the video
            style (str): Style of the video (photorealistic, digital art, etc.)
            voice_style (str): Voice style for the TTS system
            max_duration (int): Maximum duration in seconds
            background_queries (list): Section-specific background queries
            blur_background (bool): Whether to blur background images
            edge_blur (bool): Whether to add an edge blur effect
            add_watermark_text (str): Optional watermark text
        ...
        try:
            ...
            # Check for custom background path or pre-processed clips
            if custom_background_clips and len(custom_background_clips) > 0:
                ...
                # Skip image preparation since we have clips already
                skip_image_preparation = True
            else:
                skip_image_preparation = False
                
                # Check for a custom background path that we should use for all sections
            ...
            # Use background clips if we have them (from custom_background_clips)
            if skip_image_preparation:
                background_clips = custom_background_clips
            else:
                # Process background images into clips
                background_clips = []
                
                # Create background clips in parallel
                background_info = []
                ...
                background_clips = self._process_background_images_parallel(
                    background_info,
                    blur_background=blur_background,
                    edge_blur=edge_blur
                )
    ...
    @measure_time
    def _process_background_images_parallel(self, background_info, blur_background=False, edge_blur=False, max_workers=None):
        """
        Process background images in parallel
        
        Args:
            background_info: List of dicts with image_path, duration, and section_idx
        ...
        """
        import concurrent.futures
        from helper.image import create_image_clip_with_zoom
        from helper.blur import custom_blur, custom_edge_blur
        from helper.memory import optimize_workers_for_rendering
        
        # Get optimal worker count
        resource_config = optimize_workers_for_rendering(memory_per_task_gb=0.5)  # Image processing is less memory intensive
        worker_count = max_workers or resource_config.get('worker_count', 3)
        
        logger.info(f"Processing {len(background_info)} background images with {worker_count} workers")
    ...
    def _process_single_image(self, image_path, duration, section_idx, blur_background, edge_blur):
        ...
        try:
            ...
            clip = create_image_clip_with_zoom(
            ...
            clip._debug_info = f"Background image {section_idx}"
            ...
            logger.error(f"Error processing image {image_path} for section {section_idx}: {e}")
            return None
