...
class YTShortsCreator_V:
    ...
    def create_youtube_short(self, title, script_sections, background_query="abstract background",
                            ...
                            existing_audio_data=None, custom_background_path=None, custom_background_clips=None):
        """
        ...
        Args:
            ...
            custom_background_path (str): Path to custom background file (will be ignored if custom_background_clips provided)
            custom_background_clips (list): List of pre-processed custom background clips
        ...
        try:
            ...
            if not videos_by_query and not custom_background_clips:
                logger.error("No background videos fetched and no custom backgrounds provided")
                return None
            ...
            # 4. Create background clips
            logger.info("Processing background videos")
            background_clips = []
            
            # Use custom background clips if provided
            if custom_background_clips and len(custom_background_clips) > 0:
                logger.info(f"Using {len(custom_background_clips)} provided custom background clips")
                background_clips = custom_background_clips
            else:
                # Process background videos based on query results
                background_clips = self._prepare_background_videos(
                    script_sections=script_sections,
            ...
            # Check if we have any background clips
            if not background_clips:
                logger.error("Failed to process background videos")
                return None
    ...
    def _prepare_background_videos(self, script_sections, videos_by_query, background_queries, 
                           blur_background=False, edge_blur=False, custom_background_path=None):
        """
        Prepare background videos for each section
        
        Args:
            script_sections: List of script section dictionaries
            videos_by_query: Dictionary mapping queries to video paths
            background_queries: List of background queries for each section
        ...
        # Check for custom background path first
        if custom_background_path and os.path.exists(custom_background_path):
            logger.info(f"Using custom background video: {custom_background_path}")
            
            try:
                # Load the video
                with VideoFileClip(custom_background_path) as source_clip:
                    # Prepare video info for parallel processing
                    video_info = []
                    
                    # Calculate optimal segments based on total duration
                    total_video_duration = source_clip.duration
                    total_needed_duration = sum(section.get('duration', 5) for section in script_sections)
                    
                    for i, section in enumerate(script_sections):
                        target_duration = section.get('duration', 5)
                        
                        # If video is long enough, use different segments for variety
                        if total_video_duration > total_needed_duration * 1.2:
                            # Calculate start position to evenly distribute across video
                            segment_percent = i / len(script_sections)
                            start_time = min(segment_percent * total_video_duration, 
                                           total_video_duration - target_duration - 0.5)
                        else:
                            # Just start from beginning if video isn't long enough
                            start_time = 0
                        
                        # Create a subclip for processing
                        end_time = min(start_time + target_duration + 0.5, total_video_duration)
                        subclip = source_clip.subclip(start_time, end_time)
                    ...
                    # Check for success
                    if background_clips:
                        logger.info(f"Successfully prepared {len(background_clips)} background clips from custom video")
                        return background_clips
                    else:
                        logger.warning("Failed to process custom background video, falling back to search results")
            except Exception as e:
                logger.error(f"Error processing custom background video: {e}")
                logger.warning("Falling back to search results for backgrounds")
        
        # Use fetched videos if no custom background or processing failed
        video_paths = []
