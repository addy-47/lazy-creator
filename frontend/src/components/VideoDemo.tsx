...
const VideoDemo = () => {
  ...
  useEffect(() => {
    ...
    // Handle theme changes from Navbar
    ...
    const drawDemoFrame = () => {
      ...
      // Use updated themeRef for background color
    ...
    const drawStickFigure = (
      ...
      if (phase === 2) {
        ...
        ctx.moveTo(x, y + 7);
        ctx.lineTo(
          x + Math.sin(legAngle) * 10,
          y + 15 + Math.cos(legAngle) * 3
        );
        ctx.stroke();
    ...
    // Cleanup: Cancel animation frame and clear canvas when effect re-runs
    return () => {
